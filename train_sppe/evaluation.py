...
from utils.img import vis_frame
...
gaussian_kernel = nn.Conv2d(
    17, 17, kernel_size=4 * 1 + 1, stride=1, padding=2, groups=17, bias=False
)
...
def prediction(model):
    ...
    for i, (inp, box, im_name, metaData) in enumerate(minival_loader_desc):
        ...
        if im_name[0] in tmp_inp.keys():
            ...
            tmp_inp[im_name[0]]["ori_inps"] = torch.cat((ori_inps, ori_inp), dim=0)
