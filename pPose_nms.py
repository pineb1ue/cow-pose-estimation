...
def pose_nms(bboxes, bbox_scores, pose_preds, pose_scores):
    ...
    pose_preds:     pose locations list (n, 5, 2)
    pose_scores:    pose scores list    (n, 5, 1)
    """
...
def p_merge(ref_pose, cluster_preds, cluster_scores, ref_dist):
    ...
    INPUT:
        ref_pose:       reference pose          -- [5, 2]
        cluster_preds:  redundant poses         -- [n, 5, 2]
        cluster_scores: redundant poses score   -- [n, 5, 1]
        ref_dist:       reference scale         -- Constant
    OUTPUT:
        final_pose:     merged pose             -- [5, 2]
        final_score:    merged score            -- [5]
    """
    dist = torch.sqrt(
...
def get_parametric_distance(i, all_preds, keypoint_scores, ref_dist):
    ...
    score_dists = torch.zeros(all_preds.shape[0], 5)
